...
class VoiceManager {
    ...
    displayVoices() {
        ...
        const premiumVoices = this.allowedVoiceNames
            .map((name) => this.voices.find((voice) => voice.name === name))
            .filter(Boolean);

        if (!premiumVoices.length) {
            grid.innerHTML = `<div class="voice-desc">Google premium voices are not available in this browser.</div>`;
            this.selectedVoice = null;
            return;
        }
        ...
        premiumVoices.forEach((voice) => {
            html += `<div class="voice-card" data-voice-name="${voice.name}"><div class="voice-name">${voice.name}</div><div class="voice-desc">${voice.lang} - Cloud</div><div class="voice-badge badge-premium">GOOGLE PREMIUM</div></div>`;
        });
        grid.innerHTML = html;
        ...
        const selectedStillAvailable = premiumVoices.some((voice) => this.selectedVoice && voice.name === this.selectedVoice.name);
        if (!selectedStillAvailable) this.selectVoice(premiumVoices[0].name, false);
    }
