...
function updateChapterSelection() {
    ...
    document.querySelectorAll("[data-chapter-index]").forEach((item) => {
        ...
        item.classList.toggle("active", itemIndex === activeChapterIndex);
    });
}
...
function setupIOSEdgeTTS() {
    ...
    edgeTTS.onBoundary = (_word, offset) => {
        if (!appState.iosPlaybackActive) return;
        const text = getCurrentReadableText();
        if (!text || !text.length) return;
        const relative = Number.isFinite(offset) ? Math.floor(offset) : 0;
        const absolute = clamp(appState.iosPlaybackChunkStart + relative, 0, Math.max(text.length - 1, 0));
        appState.readingStartIndex = absolute;
        voiceManager.totalLength = text.length;
        voiceManager.updateProgress(absolute);
        updateStartPositionLabel();
        updateChapterSelection();
    };
