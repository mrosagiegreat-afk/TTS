...
class EdgeTTS {
    ...
    async speak(text, voice = this.currentVoice, rate = 1.0) {
        if (!text) return;
        this.stop();
    ...
    fallbackToIOS(text, rate = 1.0) {
        console.warn("Edge TTS failed, using iOS fallback");
        const utterance = new SpeechSynthesisUtterance(text);
        const fallbackVoice = voiceManager && voiceManager.selectedVoice ? voiceManager.selectedVoice : null;
        if (fallbackVoice) utterance.voice = fallbackVoice;
        utterance.rate = clamp(rate, 0.5, 2.0);
        window.speechSynthesis.speak(utterance);
    }
}
...
function setupIOSEdgeTTS() {
    ...
    edgeTTS.onBoundary = () => {
        // Reserved for iOS word-level highlighting integration.
    };
}

function isAnyReaderPlaybackActive() {
    const edgeSpeaking = Boolean(isIOSDevice && edgeTTS && edgeTTS.isSpeaking);
...
function readAloud() {
    ...
    if (isIOSDevice && edgeTTS) {
        ...
        edgeTTS.speak(textToRead, "en-US-JennyNeural", rate);
        return;
    }
...
function pauseReading() {
    if (isIOSDevice && edgeTTS && edgeTTS.isSpeaking) {
        edgeTTS.stop();
        autoScroll.stopAutoScroll();
        document.getElementById("voiceWave").style.display = "none";
        saveCurrentPosition();
...
function handleKeyboardShortcuts(event) {
    ...
    if (!event.ctrlKey && !event.altKey && !event.metaKey && event.key === " ") {
        event.preventDefault();
        if (isIOSDevice && edgeTTS && edgeTTS.isSpeaking) pauseReading();
