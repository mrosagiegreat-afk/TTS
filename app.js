...
class VoiceManager {
    ...
    displayVoices() {
        ...
        if (this.isIOS) {
            ...
            voicesToShow = this.voices.filter((v) =>
                siriVoiceNames.some((name) => (v.name || "").toLowerCase().includes(name.toLowerCase()))
            );
        ...
        voicesToShow.forEach((voice) => {
            const isSiri = this.isIOS && /samantha|aaron|allison|siri|tom|martha|ava|nick|daniel|karen|moira|tessa|veena/i.test(voice.name || "");
            const badgeClass = isSiri ? "badge-neural" : "badge-standard";
            const badgeText = isSiri ? "SIRI NEURAL" : "IOS VOICE";
        ...
        const defaultVoice = voicesToShow.find((v) => /samantha|aaron|allison|siri/i.test(v.name || "")) || voicesToShow[0];
        if (defaultVoice) {
