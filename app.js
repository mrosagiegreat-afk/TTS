...
class EdgeTTS {
    constructor() {
        ...
        this.endpoint = "https://edge-tts-server-9c8d7a9f9c8d.herokuapp.com/synthesize";
    ...
    async speak(text, voiceId = this.currentVoice, rate = 1.0) {
        ...
        try {
            ...
            if (requestId !== this.requestId || !this.isSpeaking) return;
    ...
    fallbackToSystemVoice(text, rate) {
        console.warn("Using system voice as fallback");
        this.stop(true);
        this.isSpeaking = true;
...
let iosProgressTimer = null;

function stopIOSProgressFallback() {
    if (iosProgressTimer) {
        clearInterval(iosProgressTimer);
        iosProgressTimer = null;
    }
}

function startIOSProgressFallback() {
    stopIOSProgressFallback();
    const text = getCurrentReadableText();
    if (!text || text === PLACEHOLDER_TEXT) return;

    voiceManager.totalLength = text.length;
    const charsPerSecond = Math.max(6, 14 * parseFloat(document.getElementById("rate").value));
    iosProgressTimer = window.setInterval(() => {
        if (!edgeTTS || !edgeTTS.isSpeaking || !appState.iosPlaybackActive) return;
        const current = Number.isFinite(voiceManager.currentAbsoluteChar)
            ? voiceManager.currentAbsoluteChar
            : appState.readingStartIndex;
        const next = clamp(Math.floor(current + (charsPerSecond * 0.2)), 0, Math.max(text.length - 1, 0));
        voiceManager.updateProgress(next);
    }, 200);
}
...
function setupIOSEdgeTTS() {
    ...
    edgeTTS.onStart = () => {
        ...
        if (appState.iosPlaybackActive) startIOSProgressFallback();
    };
    edgeTTS.onEnd = () => {
        const wave = document.getElementById("voiceWave");
        if (wave) wave.style.display = "none";
        if (appState.iosPlaybackActive) {
    ...
    const initAudio = () => {
        if (!edgeTTS) return;
        if (!edgeTTS.audioContext) {
            edgeTTS.audioContext = new (window.AudioContext || window.webkitAudioContext)();
        }
    };
    document.addEventListener("touchstart", initAudio, { once: true });
    setTimeout(setupIOSVoiceGrid, 1000);
}
...
function setupIOSVoiceGrid() {
    ...
    document.querySelectorAll("[data-edge-voice]").forEach((card) => {
        card.addEventListener("click", () => {
            ...
            edgeTTS.speak("Hello, I am ready to read.", card.dataset.edgeVoice, 0.9);
            closeHeaderMenus();
        });
    });
}
...
function iosReadAloud() {
    ...
    const textToRead = text.slice(safeStart).trim() || text;
    voiceManager.totalLength = text.length;
    voiceManager.updateProgress(safeStart);
    if (edgeTTS.audioContext && edgeTTS.audioContext.state === "suspended") {
        edgeTTS.audioContext.resume().catch(() => {});
    }
    edgeTTS.speak(textToRead, edgeTTS.currentVoice, parseFloat(document.getElementById("rate").value));
    closeHeaderMenus();
}
